...
CREATE TABLE IF NOT EXISTS `supervisiona` (`veterinarioCPF` VARCHAR(255) NOT NULL REFERENCES `veterinario` (`CPF`) ON DELETE CASCADE ON UPDATE CASCADE, `especieId` INTEGER NOT NULL, `createdAt` DATETIME NOT NULL, `updatedAt` DATETIME NOT NULL, `animalId` INTEGER REFERENCES `especie` (`id`) ON DELETE CASCADE ON UPDATE CASCADE, PRIMARY KEY (`veterinarioCPF`, `especieId`));
